...
begin
	...
	using GeoDataFrames
	using Statistics
	using UnicodePlots	# augmentation::Union{DataFrame, Vector{}},
	# join_on::Union{String, Symbol, Vector{String}, Vector{Symbol}},
	using YAML
end;

# ╔═╡ ce4fab08-7af7-453e-97c3-5a12992e6497
begin
Pkg.add("XGBoost")
using XGBoost
end
...
# ╔═╡ 8a4a6ec0-c025-45b1-9e02-90fed434d091
using Logging
...
# ╔═╡ 86dcf0b8-eea4-4b6b-a28b-857db016babd
begin
Pkg.add("MLJXGBoostInterface")
XGBoostRegressor = @load XGBoostRegressor pkg=XGBoost verbosity=1
end
...
# ╔═╡ 86354fbf-bc54-470e-9fa7-b2722bf7de9d
using EvoTrees
...
# ╔═╡ cacfed65-3485-4785-bc7b-a4420ad05070
begin
...
# ╔═╡ 011bad88-80c9-42a7-b259-1735ce1cc630
...
# ╔═╡ ce1a5d3e-f348-4d71-b306-63a6f369ba45
unique_buildings = unique(energydata[:,"Property Id"])

# ╔═╡ 2cb99937-b1b2-4b89-8858-fdd974dc980b
test_buildings = sample(
	unique_buildings, 
	trunc(Int, length(unique_buildings) * 0.15)
);

# ╔═╡ 77b16a35-fec1-4975-b1f8-4736354aff75
length(unique_buildings)

# ╔═╡ 73e617e3-c7d8-4792-ad41-078c3fb54082
length(test_buildings)

# ╔═╡ 21fbb8b3-dda5-480a-a4d2-a3833e81fb3c
length(setdiff(unique_buildings, test_buildings))

# ╔═╡ 9aa06073-d43e-4658-adb9-bbc11425978d
begin
	train, test = splitdf(energydata, 0.85);
	# train, validate = splitdf(energydata, 1 - 0.176);

	train = coerce(energydata, 
		:zone => Multiclass, 
...
# begin
# unique_buildings = unique(train[:,"Property Id"])

# # this is the sample process I'm going to use when we do the cross validation
# sample_buildings = sample(unique_buildings, 100)
# building_boolmap = map( x -> x ∈ sample_buildings, train[:,"Property Id"] );
# building_rows = collect(1:nrow(train))[building_boolmap]
# end;

# ╔═╡ c8a8830a-a861-4b35-ae13-2a5cccecbe50
building_zones = select(unique(train, Symbol("Property Id")), [Symbol("Property Id"),:zone]);
...
# ╔═╡ 4458848e-724e-4390-a0f8-149ad597f653
...
# ╔═╡ dc8c5f95-2987-4d8d-8936-2f89816e3060
function monthly_aggregation(
	data::DataFrame, 
	agg_terms::Vector{String},
...
# ╔═╡ a8811cb8-4177-4bbe-8b49-49f6f656d76c
sar
...
sample_sars = sample(unique(sar′[:,"Property Id"]), 100)
...
# ╔═╡ 71a9c9d1-efee-414d-a6a7-2fc8e182dd1e
epw_temps = unique(select(epw_r, ["date","Drybulb Temperature (°C)"]))

# ╔═╡ 7c1a1907-e31e-4dff-a789-cc31e0766ea7
date1 = Date(2018,1,1):Month(3):Date(2020,12,1)
...
t₁ = Gadfly.plot(
	...
	Gadfly.layer(
		...
		Theme(line_width=2pt, default_color="black")
		# intercept=[0], 
		# slope=[1],
	...
	Gadfly.layer(
		...
		# Geom.point,
		# Geom.line,
		Geom.smooth(smoothing=0.12),
		Theme(line_width=0.1pt, alphas=[0.5])
	),
	Scale.discrete_color_hue,
	Guide.ylabel("Land Surface Temperature - °C"),
	Guide.xlabel("Date"),
	Guide.xticks(ticks=DateTime("2018-01-1"):Month(1):DateTime("2020-12-31")),
	Guide.title("Land Surface Temperature Inconsistencies - Landsat8"),
...
# ╔═╡ d3e9f404-0816-4ae6-be4e-ebd25c92792c
...
#  	device="gpu"
#  )
# m = EnsembleModel(model=xgmodel, n=10, rng=rng)
# end
...
# ╔═╡ e02c5103-7fb2-4ebe-bd2f-319920cc04a7
...
# ╔═╡ 149afb6c-c34a-478e-88df-76f44310248f
DataFrame(importancetable(bst))

# ╔═╡ d2667629-d450-4542-b039-65a0e72dfdb4
begin
a = DataFrame(randn(100,2), [:a,:b])
b = DataFrame(randn(100,2), [:a,:b])
...
# ╔═╡ 174900ad-0218-47ea-b60e-92f5458878dd
begin
	hcat(Matrix(a),Matrix(b))
end

# ╔═╡ bc068772-e897-4cf7-ac03-607db70d2772
ismissing.(tₐ′.VV)

# ╔═╡ b489eb56-eb19-42d6-8607-757776d721bd
sum(isnan.(tₐ′.VV))

# ╔═╡ 871867ca-96a9-4e7a-8f1d-0be901d00aca
#hyperparams for the cross validation
num_rounds = 7

# ╔═╡ 5d65f55e-4c53-4d27-af17-ddf93a925a43
max_depth = 6

# ╔═╡ d41ca22b-6d0a-4d1b-8b41-f04b63b887d0
cv_fraction = 0.2

# ╔═╡ d789ef8d-2262-4f13-9714-1c9ea0fa23a2
@warn "youtube"

# ╔═╡ 13bb5ae2-d3c3-4202-be20-4511b5da3873
Logging.disable_logging(Logging.Info)

# ╔═╡ 20fd0391-7a61-4b7e-8fe7-7051c832a974
function electric_cv(terms)
termtest = select(tₐ′, terms)
select!(termtest, Not(joining_terms))

features = select(termtest, Not([:electricity_mwh,:weather_station_distance]))
predictor = termtest.electricity_mwh

unique_buildings = unique(tₐ′[:,"Property Id"])
cv_size = trunc(Int, length(unique_buildings) * cv_fraction)

# alright now want to make a list of rows to pull
resampling_trial::Vector{Tuple{Vector{Int64}, Vector{Int64}}} = []
...
for i in 1:3
	...
	building_cv = collect(1:nrow(termtest))[building_boolmap]
	building_train = collect(1:nrow(termtest))[.!building_boolmap]
	push!(resampling_trial, (building_train, building_cv))
end

importance_total_dict = Dict()
importance_counts = []
errors = []
for sampling_trial in resampling_trial
	trial_train, trial_test = sampling_trial
	train_x = features[trial_train,:]
	train_y = predictor[trial_train]

	test_x = features[trial_test,:]
	test_y = predictor[trial_test]
	...
	importance_list = XGBoost.importance(bst)
	importance_keys = keys(importance_list)
	importance_counts = [importance_counts..., importance_keys...]
	for importance_key in importance_keys
		if importance_key ∉ collect(keys(importance_total_dict))
			importance_total_dict[importance_key] = importance_list[importance_key]
		else
			importance_total_dict[importance_key] += importance_list[importance_key]
		end
	end	

	ŷ = XGBoost.predict(bst, test_x)
	push!(errors,rmse(ŷ, test_y))
end

term_counts = countmap(importance_counts)
println(term_counts)
for term in keys(term_counts)
	println(term)
	importance_total_dict[term] /= term_counts[term]
end

df = DataFrame(importance_total_dict)
importancedf = DataFrame([[names(df)]; collect.(eachrow(df))], [:column; Symbol.(axes(df, 1))])
rename!(importancedf,[:feature, :gain])
...
return bst, (mean(errors), std(errors)), sort(importancedf, :gain, rev=true)
end

# ╔═╡ 0fb90202-f6ad-4715-9244-9198f776e4c0
bst₆, err₆, imp₆ = electric_cv(term₆);

# ╔═╡ 093cd667-23b1-4c83-a675-b8da9369914f
df = DataFrame(imp₆)

# ╔═╡ 929860c3-5a8e-401b-abe5-45fd240133c9
uuu = DataFrame([[names(df)]; collect.(eachrow(df))], [:column; Symbol.(axes(df, 1))])

# ╔═╡ 52436244-6cf5-4452-baf8-9d53967f16b2
rename(uuu, [:feature, :gain])

# ╔═╡ 9e8bb14f-8cfe-4ea0-b2ba-c2e1ff703e5f
bs, err, importancetables = electric_cv(term₇);

# ╔═╡ 9eacf4dd-7f2c-487e-8321-2f46e94f78d2
DataFrame(XGBoost.importancetable(bs))

# ╔═╡ d5d7397c-bf88-43a6-8345-821afcf88cae
importancetables
...
l₃ = Gadfly.layer(validate, x=:weather_station_distance, Geom.density, Theme(default_color="orange"))
	
l₃ = Gadfly.plot(l₁, l₂, l₃, Scale.x_continuous(minvalue=0, maxvalue=2.5e4),
	Coord.Cartesian(xmin=0,xmax=2.5e4)
)
end
...
function electricaggregation(individual_results)
	...
	ṫ = combine(groupby(t, ["date"]), [:prediction, :recorded] .=> sum, :model => first, renamecols=false)
