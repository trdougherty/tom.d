...
# ╔═╡ ec19bc14-5ce6-40ee-89b7-d02295eb5c4e
using ArchGDAL

# ╔═╡ ed03cae9-fac6-46a1-8605-b74513379c76
using GeoArrays

# ╔═╡ 447afd7d-698e-41c0-b15a-c931f6ed7af2
using ImageFiltering

# ╔═╡ 66d9c6f7-a693-4311-b00f-178ae86d593b
using Images

# ╔═╡ 41221d20-6529-11ed-2203-d929e09495cd
using Rasters

# ╔═╡ 136c14f4-cc2c-45b9-ad15-c17a8e60d8e8
using RasterDataSources

# ╔═╡ 338f6fdd-2c4c-4ee1-a21d-ba72a4d21027
using Plots

# ╔═╡ e530edbc-e93b-4759-82bb-d39f34dfcea1
using Statistics

# ╔═╡ b8097367-260e-47e0-af87-08a375f3fd2f
using Shapefile

# ╔═╡ 0ae89e05-1ffc-42a7-9d21-488b8a769468
import GeoDataFrames as GDF

# ╔═╡ f6961517-9338-4eef-afa0-df74c52ef770
import GeoFormatTypes as GFT

# ╔═╡ 20c0224e-2544-45d2-a77b-2f3f738b5a21
temps = Raster("sar_nyc.tif")

# ╔═╡ d9513198-ba07-400b-b442-65946879b70c
small_temps = temps[:,:,1]

# ╔═╡ 038b5ccc-74b1-4d86-bd90-cc61d29df58b
GFT.WellKnownText(crs(small_temps).val)

# ╔═╡ bec94aa4-2b24-4225-a66a-2447ddbf8b2f


# ╔═╡ f889f4c7-90b3-46ec-9164-b5f398a33159
ga_test = GeoArrays.read("sar_nyc.tif")

# ╔═╡ 4f55f00c-22ee-48e0-aecd-9d0e7f3ee86a
temp_proj = Rasters.reproject(EPSG(3857), EPSG(4326), 3, temps)

# ╔═╡ fe8911a4-4339-4b95-ba45-3909e9d55f6e


# ╔═╡ de471171-a1fa-4920-9ec4-03e2a9504083


# ╔═╡ bc43bb0c-b0db-430b-afc2-15a55e0622de
plot(small_temps, max_res=4000)

# ╔═╡ 64471008-5223-42ed-a325-c7c2fe43b2f2
imgg = imfilter(small_temps, Kernel.gaussian(100));

# ╔═╡ 6e9111b7-7dd5-4303-9427-6a704d44873c
...
# ╔═╡ a5a804a5-17cf-48cd-b5b5-098d961f7747
size(small_temps)

# ╔═╡ 0a696cfc-16d7-4503-834c-9a3a570eee15
begin
local_zones = GDF.read("geo_export_927bcc4e-9baf-479d-b10d-3ac6afbb3a67.shp")
local_zones.geometry = GDF.reproject(
	local_zones.geometry, 
	GFT.EPSG(4326), 
	GFT.WellKnownText(crs(small_temps).val),
	order=:trad
)
...
# ╔═╡ 667ce6a0-a7e9-45fd-a93a-38ffb5ce1ca7
begin
plot(local_zones.geometry, color=:transparent, dpi=600)
contourf!(imgg, linewidth=1.0, alpha=0.09, title="VH Absorption", max_res=200)
end
...
julia_version = "1.8.2"
manifest_format = "2.0"
project_hash = "5f585d8f9853e5386122265e459a44f88fe738ec"
...
git-tree-sha1 = "ea52547d6aab650e9120b95d2c2dec13bd747436"
...
[[deps.GDAL_jll]]
deps = ["Artifacts", "Expat_jll", "GEOS_jll", "HDF5_jll", "JLLWrappers", "LibCURL_jll", "LibPQ_jll", "Libdl", "Libtiff_jll", "NetCDF_jll", "OpenJpeg_jll", "PROJ_jll", "Pkg", "SQLite_jll", "Zlib_jll", "Zstd_jll", "libgeotiff_jll"]
git-tree-sha1 = "ac63c49efd5afe2054b044926c7f112c1cb31d25"
uuid = "a7073274-a066-55f0-b90d-d619367d196c"
version = "300.500.200+0"
...
git-tree-sha1 = "51b0bd6cc8c61d303c4e59b5214376c098d2c370"
...
git-tree-sha1 = "d09655445e2136d103fefbdaaace93c82041a524"
...
git-tree-sha1 = "ad47689251a391fa2419a3cb8f6c1837e285c7e2"
...
deps = ["AbstractTrees", "CategoricalArrays", "Distributions", "LinearAlgebra", "Optim", "PrettyTables", "Random", "ScientificTypes", "Statistics", "StatsBase", "Tables", "Transducers", "TransformsBase"]
git-tree-sha1 = "835afd5be845a66f089f0ade41ebeeb1bfb2cf00"
